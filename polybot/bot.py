...
class Bot:
    ...
    def __init__(self, token, telegram_chat_url):
        ...
        self.telegram_bot_client.set_webhook(url=f'{telegram_chat_url}/{token}/',
                                             certificate=open('cert.pem', 'r'), timeout=60)
    ...
    def handle_message(self, msg):
        ...
        try:
            ...
            #upload the photo to S3
                s3.upload_file(img_path,s3_bucket,obj_name)
                logger.info(f'successfully uploaded to s3')
            #send a job to the SQS queue

                sqs_client = boto3.client('sqs',region_name='eu-central-1')
                sqs_request = f'{obj_name} {chat_id}'
                sqs_client.send_message(QueueUrl=queue_name, MessageBody=sqs_request)
            elif "text" in msg:
